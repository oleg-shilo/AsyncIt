...
public class TestBase
{
    ...
    public (int exitCode, string output) BuildWithDotNet(string testCode, string code, string autogeneratedCode, [CallerMemberName] string context = "")
    {
        var folder = new TestFolder(context);
        folder.Create("test.csproj");
        folder.Copy(typeof(CodeGenerator).Assembly.Location);

        folder.Create("Models.cs");
        folder.Create(testCode, "Program.cs");
        folder.Create(code, "service.cs");
        folder.Create(autogeneratedCode, "service.g.cs");

        return folder.ExecuteBackgroundProcess("dotnet.exe", "build");
    }
}
...
static class TestExtensions
{
    ...
    public static string GenerateExtraSource(this string code, AsyncAttribute attribute = null)
        => CSharpSyntaxTree.ParseText(code).GenerateSourceForTypes(attribute ?? new AsyncAttribute()).First().Value;
...
class TestFolder
{
    ...
    public TestFolder(string name)
    {
        ...
        foreach (var item in Directory.GetFiles(ProjectDir))
        {
            File.Delete(item);
        }
    }
    ...
    internal string ProjectDir
    {
        get
        {
            ...
            projectDir = Path.Combine(Path.GetDirectoryName(thisAsm), "system-test", name);
    ...
    {
        var src = Path.Combine(SourcesDir, fileName);
        var dest = Path.Combine(ProjectDir, fileName);
        File.Copy(src, dest, true);
        return dest;
    }

    public string Create(string code, string fileName)
    {
        var dest = Path.Combine(ProjectDir, fileName);
    ...
    public void ProcessWithAsyncIt(string fileName, AsyncAttribute attr = default)
    {
        var code = File.ReadAllText(Path.Combine(ProjectDir, fileName));
        var newCode = CSharpSyntaxTree.ParseText(code).GenerateSourceForTypes(attr).First().Value;
        File.WriteAllText(Path.Combine(ProjectDir, Path.ChangeExtension(fileName, ".g.cs")), newCode);
    }

    public (int exitCode, string output) ExecuteBackgroundProcess(string app, string args)
    {
