...
public static class ISymbolExtensions
{
    ...
    public static string GetParametersString(this IMethodSymbol method, string memberSeparator = "")
    {
        var prms = "";
        if (method.Parameters.HasAny())
            prms = GetParametersString(method.OriginalDefinition.Parameters, memberSeparator);

        if (prms.Any() && method.IsExtensionMethod)
            prms = "this " + prms + memberSeparator;
    ...
    public static string GetParametersString(this IEnumerable<IParameterSymbol> parameters, string memberSeparator = "")
    {
        if (parameters.HasAny())
        {
            string prms = string.Join(", ", parameters
                                                  .Select(p =>
                                                  {
                                                      ...
                                                      return $"{refKind}{p.Type.ToMinimalString()} {p.Name}{defValue}{memberSeparator}";
                                                  })
                                                  .ToArray());
            ...
            foreach (var item in typeParameters)
            {
                ...
                if (constrains.HasAny())
                    if (singleLine)
                        ...
                        code.Append(Runtime.NewLine + "    " + constrains);
            }
        return code.ToString();
    }
